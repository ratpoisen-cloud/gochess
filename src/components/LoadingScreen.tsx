...
export default function LoadingScreen({ isLoading }: LoadingScreenProps) {
  ...
  return (
    ...
    >
      <div
        className="w-[min(92vw,420px)] p-[32px_24px] rounded-[var(--radius-8)] border border-[color-mix(in_srgb,var(--accent-brand)_30%,var(--border))] text-center"
        style={{
          background: 'var(--bg)',
        }}
      >
        ...
        <div className="mt-5 inline-grid grid-flow-col gap-2.5 justify-center">
          {[0, 1, 2, 3].map((i) => (
            <span
              key={i}
              className="w-2.5 h-2.5 bg-[var(--accent-brand)] border border-[color-mix(in_srgb,var(--accent-brand)_40%,transparent)] shadow-[0_0_10px_color-mix(in_srgb,var(--accent-brand)_30%,transparent)] animate-[app-loading-pixel-steps_1s_steps(4,end)_infinite]"
              style={{
