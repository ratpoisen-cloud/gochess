...
- Реализовано через `ReactionPicker` (8 колонок, ретро-стиль) и `reactionStore`.
- Анимация: `animate-bounce`, время жизни: 5000мс.
- Лимит: 5 эмодзи на игрока за полуход (всего 10 за полный ход).

## Игровые запросы
- **Отмена хода (Undo):** Откатывает 1 полуход с сохранением истории (через PGN).
- **Ничья (Draw):** Предложение и принятие ничьей по соглашению.
- **Реванш (Rematch):** Создание новой комнаты с инверсией цветов игроков.
- Реализовано через всплывающие модальные окна с полупрозрачным фоном и размытием.

## Интерфейс и Брендинг
- **Цвета:** Основной зеленый `#7eb87e` (логотип), системный молочный `#e8e8d8` (текст и границы).
- **Лобби:** Кинематографичная заставка (typing effect), мини-превью досок (100px), отображение статуса «Ваш ход» / «Ход соперника».
- **Игровая комната:** Унифицированный статус-бар (3 колонки: соперник, уведомления, статус хода).
- **Превращение пешки:** Интегрированный вертикальный пикер прямо на доске, привязанный к целевой клетке. Использует `var(--radius-14)` и полупрозрачный фон.
- **Инвайты:** Упрощенный модуль в сайдбаре с кнопками Телеграм/ВК и клик-копированием ссылки.
- **Модалки:** Стандартизированный дизайн настроек (полноширинные кнопки, красная подсветка «Отмена» на hover).
- **Пользователь:** Отображение имени игрока в текстовом формате (без аватаров) для соответствия минималистичному ретро-стилю.
...
### 🟡 Средние
1. **Responsive** — боковая панель не уходит под доску на мобильных

### 🟢 Низкие
2. **Бот** — случайные ходы вместо Stockfish
3. **Supabase** — не полностью интегрирован (часть состояния локальна)
